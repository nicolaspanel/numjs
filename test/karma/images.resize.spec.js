...
describe('images', function () {
  describe('resize', function () {
    ...
    it('should be able to downscale a Grayscale img', async function () {
      expect(gray.shape).to.eql([28, 28]);
      var resized = await nj.images.resize(gray, 14, 10);
    ...
    it('should be able to upscale a Grayscale img', async function () {
      expect(gray.shape).to.eql([28, 28]);
      expect(await nj.images.resize(gray, 32, 30).shape).to.eql([32, 30]);
    });

    it('should be able to downscale an RGB img', async function () {
      expect(rgba.shape).to.eql([300, 600, 4]);
      expect(await nj.images.resize(rgba, 150, 300).shape).to.eql([
        150, 300, 4,
      ]);
    });
  });
